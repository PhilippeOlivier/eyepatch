...
ntfy_topic="eyepatch42" # ntfy topic to subscribe to on the mobile app
track_file="/full/path/of/eyepatch.track"
log_file="/full/path/of/eyepatch.log"
...
tpb_names() {
	...
	local terms=$(echo "$terms_raw" | tr [:upper:] [:lower:] | tr ' ' '+')
	local results=$(curl -s "https://pirateproxy.surf/api?url=/q.php?q={$terms}&cat=")
	# If Cloudflare blocks the search, don't parse the results with `jq`.
	if echo "$results" | grep -qi "cloudflare"; then
		results="cloudflare"
	else
		results="$(echo $results | jq .[].name)"
	fi
	echo "$results"
}
...
episode_exists() {
	...
	if echo "$names" | grep -q "^\"No results returned\"$"; then
	    false
	elif echo "$names" | grep -qi "cloudflare"; then
		curl -d "Error with the \"eyepatch.sh\" script (Cloudflare prevented search for \"$(make_title "${series}") S${season}E${episode}\")." ntfy.sh/${ntfy_topic}
		false
	else
		...
		if echo "$names" | grep -qi "$expression"; then
			true
		else
			false
		fi
...
init() {
	...
	# If the tracking file does not exist, create it.
	...
	while read line; do
		if [[ ! $line =~ $format ]]; then
			sed -i "s/$line/$line 0 0/" "$track_file"
		fi
	done < "$track_file"

	# Remove apostrophes (torrent names typically don't have apostrophes).
	while read line; do
		sed -i "s/$line/$(echo "$line" | tr -d "'")/g" "$track_file"
	done < "$track_file"
	...
	if ! episode_exists "game of thrones" "1" "1"; then
		curl -d "Error with the \"eyepatch.sh\" script (TPB is probably down)." ntfy.sh/${ntfy_topic}
		exit 1
	fi
}
...
is_series_tracked() {
	...
	if grep -qiE "^$1 [1-9][0-9]? [1-9][0-9]?$" "$track_file"; then
		true
	else
		false
	fi
...
latest_tracked_season() {
	...
	local latest_season="$(grep -iE "^$1 [1-9][0-9]? [1-9][0-9]?$" $track_file | rev | cut -d ' ' -f2 | rev)"
	echo "$latest_season"
}
...
latest_tracked_episode() {
	...
	local latest_episode="$(grep -iE "^$1 [1-9][0-9]? [1-9][0-9]?$" $track_file | rev | cut -d ' ' -f1 | rev)"
	echo "$latest_episode"
}
...
get_series_from_line() {
	...
	echo "$1" | sed -E 's/^([[:print:]]*) ([1-9][0-9]? [1-9][0-9]?|0 0)$/\1/'
}
...
update_series_status() {
	...
	sed -Ei "s/^$1 ([0-9][0-9]? [0-9][0-9]?|0 0)$/$1 $2 $3/" "$track_file"
	# Log all changes.
	echo "[$(date +%Y-%m-%d)] $(make_title "$1") S$(d_to_dd "$2")E$(d_to_dd "$3")" >> "$log_file"
}
